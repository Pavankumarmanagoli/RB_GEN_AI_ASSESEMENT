...
`outputs/` contains many intermediate and diagnostic files retained for reproducibility. Start with these four reviewer-facing outputs:
...
## How to Read the Final Evaluation

The table below explains the columns in the `final_evaluation.xlsx` "Pairwise Evaluation" sheet. `final_pairwise_evaluation.csv` holds the same underlying row-level data under its raw technical field names (e.g. `row_id`, `automated_fidelity_score`, `final_fidelity_label`) rather than these reviewer-facing headers; where the CSV's wording differs meaningfully from the workbook's business-facing text, that's noted below.

| Column | Meaning |
| --- | --- |
| Row ID | The row identifier (1–30) — one row per Human–AI response pair. |
| Person ID | Which of the three participants (`c_human`, `g_human`, `s_human`) the row belongs to. |
| Question | The open-ended question asked in both interviews. |
| Human Answer | The real participant's answer, from the held-out evaluation interview. |
| AI Answer | The corresponding answer generated by that person's simulation. |
| Automated Match Score | The primary automated 1–5 score. It equals G-Eval Core Fidelity exactly (enforced by schema validation) — not a blend of the other automated signals. |
| Human-Reviewed Match Score | The reviewer's independent 1–5 score from manual review, on the same fixed rubric scale. |
| Human–AI Match | A business-facing label derived only from the Human-Reviewed Match Score, via a fixed lookup (5 → "Very strong match" … 1 → "Very weak match"). It is not derived from the automated score. (CSV: `final_fidelity_label`, shown there in its underlying technical wording, e.g. "Mixed fidelity" rather than "Partial match".) |
| Main Issue | A deterministic label built only from the three Human-review flags below — contradiction, omission, unsupported detail — with no separate score or judgment involved (e.g. omission alone → "important omission"; all three → "multiple issues"). (CSV: `main_issue`, in its underlying technical wording.) |
| Final Assessment | The reviewer's own written note, lightly grammar-corrected, with one summary line appended ("Overall match: <label>"). No new score or judgment is added. (CSV: `final_assessment_note`.) |
| Claim Coverage | Share of that row's Human claims preserved by the AI answer: (aligned + partial claims) ÷ total Human claims. Partial matches count the same as full matches — there is no fractional weighting. |
| Unsupported Rate | Share of that row's AI claims with no matching Human claim: unsupported AI claims ÷ total AI claims in that answer. The denominator is the AI's own claim count, not the Human's. |
| Claim-Based Contradiction Check | Whether claim alignment found at least one Human claim the AI answer directly contradicts, for that row. |
| NLI Contradiction Check | Whether the independent NLI cross-check found at least one matched claim pair labeled a contradiction — a separate signal from the claim-based check above, not derived from it. |
| Human Contradiction | The reviewer's own YES/NO judgment on whether the AI answer contradicts the Human answer. |
| Human Omission | The reviewer's YES/NO judgment on whether the AI answer omits something *important* from the Human answer — not any minor wording or phrasing difference. |
| Human Unsupported Detail | The reviewer's YES/NO judgment on whether the AI answer adds person-specific detail the Human reference does not support. Unsupported does not mean factually false or hallucinated — only that the Human reference didn't state it. |
